...
import { drawFrame, RAINBOW, type LabelPosition } from "@/lib/drawFrame";
...
export default function Home() {
  ...
  // 9:16 のとき、SNS の UI に隠れる範囲をプレビューにだけ薄く出す。
  ...
  const exportWithCodecs = async (seconds: number) => {
    ...
    const msPerFrame = await measureEncodeSpeed(format, width, height, (probeCtx, index) => {
      drawFrame(probeCtx, {
        ...
        background: bgImageRef.current, title, artist, labelPosition, ringState,
      });
    });
  ...
  return <div className="studio-shell">
    ...
    <main className="workspace">
      <aside className="left-rail">
        ...
        <section className="panel-section"><div className="section-label"><Library size={14} /> 情報</div><label>曲名<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="曲名を入力" /></label><label>アーティスト名<input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder="アーティスト名を入力" /></label><label>ジャンル<input value={genre} onChange={(e) => setGenre(e.target.value)} placeholder="Instrumental" /></label><div className="section-title label-position-title">文字の位置</div><div className="format-row label-position-row"><button className={labelPosition === "top" ? "active" : ""} onClick={() => setLabelPosition("top")}>上</button><button className={labelPosition === "aboveRing" ? "active" : ""} onClick={() => setLabelPosition("aboveRing")}>リングの上</button><button className={labelPosition === "bottom" ? "active" : ""} onClick={() => setLabelPosition("bottom")}>下</button></div></section>
      </aside>
